...
<form id="div1"  action="surteknisi.php" method="get" class="form-inline">
			...
			while($ip=mysqli_fetch_array($query)){
			    $data = explode('-',$ip['id_pembangunan']);
			...
			while($nt=mysqli_fetch_array($query)){
			    ...
			    $nama_t = $data[0];
			?>
			<option value="" selected disabled hidden>Pilih Teknisi</option>
			...
			<thead class="table-dark">
                <tr>
					...
					if(isset($_GET['bulan'])AND isset($_GET['tahun'])){
						...
						$thn = $_GET['tahun'];
						$query = mysqli_query($connection,"SELECT * FROM stek INNER JOIN tbkaryawan WHERE year(tgl_kp)='$thn' ORDER BY tgl_kp DESC");			
					}
					else if(isset($_GET['id_pembangunan'])){
						$ip = $_GET['id_pembangunan'];
						$query = mysqli_query($connection,"SELECT * FROM stek INNER JOIN tbpembangunan ON tbpembangunan.id_pembangunan = stek.id_pembangunan INNER JOIN tbkaryawan WHERE id_pembangunan='$ip' ORDER BY id_pembangunan DESC ");			
					}
					else if(isset($_GET['tipe'])){
						$tp = $_GET['tipe'];
						...
						$query = mysqli_query($connection,"SELECT * FROM stek INNER JOIN tbteknisi ON stek.nama_t = tbteknisi.nama_t INNER JOIN tbpembangunan ON tbpembangunan.id_pembangunan = stek.id_pembangunan INNER JOIN tbkaryawan ");
					}
					$no = 1;
